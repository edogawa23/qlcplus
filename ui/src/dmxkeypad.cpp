...
void DmxKeyPad::setupUi()
{
    ...
    this->setFixedWidth(5 * DMXKEYPAD_BUTTON_SIZE + 15);
    this->setLayout(lay);
    ...
    lay->addWidget(m_commandDisplay, 5, 0, 1, -1);
    // LineEdit appearance
    ...
    connect(m_ZERO,  SIGNAL(clicked()), this, SLOT(KP_ZERO()));
...
void DmxKeyPad::KP_ZERO()
{
    qDebug() << Q_FUNC_INFO;

    KP_AT(); // ZERO always refers to a value, not a channel. So this makes sure we end channel selection if ZERO is requested
    if (m_KPStateMachine->configuration().contains(m_KPState_Value)) {
        m_currentValue = 0;
        appendToCommand("ZERO");
        KP_ENTER(); // ZERO implies ENTER for simplicity. If you want "ZERO" and FAN function, use 0 please!
    }
}

void DmxKeyPad::KP_DOT()
{
    qDebug() << Q_FUNC_INFO;
    // not implemented
}
